...
// All rights reserved.
// Copied from https://github.dev/lf-edge/eve/blob/master/libs/zedUpload/azureutil/azure.go

// In Go, files within the same package can freely
// use functions and types declared in other files of the same package without needing explicit imports.
package main
...
// Jamal: Function that download the blob from Azure
func DownloadAzureBlob(accountURL, accountName, accountKey, containerName, remoteFile, localFile string,
	...
	// --------------------------------------------------------------------------------------------
	// Below code Modified by jamal for environment reason (windows)
	if index == -1 {
		index = strings.LastIndex(tempLocalFile, "\\")
	}
	// Above code Modified by jamal for environment reason (windows)
	// --------------------------------------------------------------------------------------------
		...
		Operation: func(chunkStart int64, count int64, ctx context.Context) error {
			...
			body = pipeline.NewResponseBodyProgress(
				...
				func(bytesTransferred int64) {
					...
					progress += diff
					// --------------------------------------------------------------------------------------------
	// Below code Modified by jamal because it was failling locally
					if progressReceiver != nil {
						progressReceiver(progress)
					}
					// Above code Modified by jamal because it was failling locally
	// --------------------------------------------------------------------------------------------
					// progressReceiver(progress)
					progressLock.Unlock()
				})
	...
	// --------------------------------------------------------------------------------------------
	// Below code Modified by jamal to make it work locally no progressReceiver
	...
	if stats.Asize < stats.Size && progressReceiver != nil {
		progressReceiver(stats.Size)
	}
	// Above code Modified by jamal to make it work locally no progressReceiver
	// --------------------------------------------------------------------------------------------
